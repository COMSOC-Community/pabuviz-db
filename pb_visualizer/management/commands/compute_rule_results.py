...
def compute_rule_results(
    election_names=None, rules=None, exact=False, overwrite: bool = False, verbosity=1
):
    ...
    for index, election_obj in enumerate(election_query):
        print_if_verbose(
            "Computing properties of election {}/{}: {}".format(
                index + 1, n_elections, election_obj.name
            ),
            1,
            verbosity,
            persist=True,
