...
class Rule(models.Model):
    ...
    description = models.TextField(blank=True,
                                   verbose_name="")
    applies_to = models.ManyToManyField(BallotType,
                                        related_name="rules")
    rule_family = models.ForeignKey(RuleFamily,
...
class Election(models.Model):
    ...
    unit = models.CharField(max_length=500,
                            ...
                            blank=True)
    subunit = models.CharField(max_length=500,
                               verbose_name="subunit",
                               help_text="name of the sub-jurisdiction",
                               blank=True)
    instance = models.SlugField(max_length=100,
                                verbose_name="instance",
                                help_text="identifier from the organizers",
                                blank=True)
    budget = models.DecimalField(max_digits=50,
                                 decimal_places=2,
                                 verbose_name="budget",
    ...
    num_projects = models.IntegerField(verbose_name="number of projects",
                                       default=0)
    ...
    has_neighborhoods = models.BooleanField(default=False,
                                            verbose_name="neighbourhoods",
                                            help_text="the voters are divided into neighborhoods")
    has_voting_methods = models.BooleanField(default=False,
    ...
    def get_meta_property(self, short_name):
        try:
            data_property = self.election_data_properties.get(metadata__short_name=short_name)
...
class Project(models.Model):
    ...
    cost = models.DecimalField(max_digits=50,
                               decimal_places=2)
    name = models.CharField(max_length=1000)  # TODO: not obligatory in pabulib standard
...
class Voter(models.Model):
    voter_id = models.CharField(max_length=1000,
    ...
    election = models.ForeignKey(Election,
                                 on_delete=models.CASCADE,
                                 related_name="voters")
    votes = models.ManyToManyField(Project,
                                   related_name="voters",
                                   through='PreferenceInfo')
    class Meta:
...
class ElectionMetadata(models.Model):
    ...
    order_priority = models.IntegerField()
    applies_to = models.ManyToManyField(BallotType,
                                        related_name="election_metadata")
...
class ElectionDataProperty(models.Model):
    ...
    metadata = models.ForeignKey(ElectionMetadata,
                                 on_delete=models.CASCADE,
                                 related_name="data_properties")
    ...
    def __str__(self):
        return "Election data property. Election: " + self.election.name + ", Metadata: " + self.metadata.name + ". Value: " + str(self.value)
...
class RuleResult(models.Model):
    ...
    selected_projects = models.ManyToManyField(Project,
                                               related_name="rule_results_selected_by")
    

    def __str__(self):
        return "Rule result. Election: " + self.election.name + ", Rule: " + self.rule.name
...
class RuleResultMetadata(models.Model):
    ...
    order_priority = models.IntegerField()
    applies_to = models.ManyToManyField(BallotType,
                                        related_name="rule_result_metadata")
