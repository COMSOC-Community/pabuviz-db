...
def get_rule_result_property_list(
    ...
    if property_short_names:
        try:
            query = RuleResultMetadata.objects.all().filter(
                short_name__in=property_short_names
            )
...
def get_filterable_election_property_list(
    ...
    def field_to_property_dict(property_short_name):
        try:
            ...
            raise ValueError(
                "Invalid property short name: {}.".format(property_short_name)
            )
    ...
    if property_short_names != None:
        for property_short_name in property_short_names:
            if property_short_name in Election.public_fields:
                ...
                property_query = ElectionMetadata.objects.all().filter(
                    short_name=property_short_name
                )
                if ballot_type != None:
                ...
                if property_query.exists():
                    properties.append(
                        ElectionMetadataSerializer(property_query.first()).data
                    )
    else:
...
def category_proportions(election_id: int, rule_abbreviation_list: str):
    try:
    ...
    for election_property in election_filters:
        if election_property == "id_list":
            election_query_set = election_query_set.filter(
                id__in=election_filters[election_property]
            )
        elif election_property == "ballot_types":
            election_query_set = election_query_set.filter(
                ballot_type__in=election_filters[election_property]
            )
